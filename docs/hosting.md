...
Los pasos completos, escritos para alguien que no programa, están en
[vercel.md](vercel.md). El resumen: subir el proyecto a GitHub, importarlo en
Vercel, crear un almacén Blob, poner `ADMIN_PASSWORD` y volver a desplegar.

Una particularidad que conviene conocer: Vercel resuelve los archivos **antes**
que las reescrituras, al revés que Netlify. Por eso el build para Vercel no
genera `dist/carta.json` — si existiera, taparía a la función y publicar desde
el panel no cambiaría nada para los clientes. De eso se encarga el
`buildCommand` del `vercel.json`, y hay una comprobación de QA que lo vigila.
