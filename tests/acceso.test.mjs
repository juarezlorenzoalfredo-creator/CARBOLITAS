...
function montarAlmacen() {
  ...
  globalThis.__CARBOLITAS_BLOBS__ = () => ({
    async getWithMetadata(key, opts = {}) {
      ...
      return { data: opts.type === 'json' ? JSON.parse(v) : v, etag: '1' };
    },
    async get(key, opts = {}) {
      if (!datos.has(key)) return null;
      const v = datos.get(key);
    ...
    async setJSON(key, value) {
      datos.set(key, JSON.stringify(value));
    },
    async delete(key) {
      datos.delete(key);
    }
...
async function cargar(password = PASSWORD, { almacen = true } = {}) {
  ...
  n += 1;
  return import(`../netlify/shared/auth.mjs?n=${n}`);
}
...
test('cambiar la contraseña invalida los tokens ya emitidos', async () => {
  ...
  delete process.env.ADMIN_SECRET;
  n += 1;
  const antes = await import(`../netlify/shared/auth.mjs?n=${n}`);
  ...
  n += 1;
  const despues = await import(`../netlify/shared/auth.mjs?n=${n}`);
  assert.equal(await despues.tokenValido(token), false);
  assert.ok(datos.size > 0);
});
...
test('revocar apaga los tokens ya emitidos', async () => {
  ...
  delete process.env.ADMIN_SECRET;
  n += 1;
  const auth = await import(`../netlify/shared/auth.mjs?n=${n}`);
  const store = await import(`../netlify/shared/store.mjs`);
  const { token } = await auth.crearToken();
  assert.equal(await auth.tokenValido(token), true);
