...
/* El mismo driver que usan el servidor de desarrollo y el QA: así las
   pruebas ejercitan el contrato real del almacén, no una imitación. */
...
async function funciones(password = 'Alitas-al-carbon-2026') {
  ...
  const [entrar, publicar, carta, foto] = await Promise.all([
    import(`../server/entrar.mjs?n=${n}`),
    import(`../server/publicar.mjs?n=${n}`),
    import(`../server/carta.mjs?n=${n}`),
    import(`../server/foto.mjs?n=${n}`)
  ]);
  return {
...
test('lo que se guarda pasa por el validador, no tal cual', async () => {
  ...
  const guardado = await almacen.leerJSON('carta.json');
...
test('sin almacén no se publica nada: se cierra, no se finge', async () => {
  ...
  assert.equal(await almacen.leerJSON('carta.json'), null);
});
...
/* ==================================================================
   lo que tiene que fallar cerrado
   ================================================================== */

/** Almacén que lee bien pero no deja escribir: cuota llena, token caducado. */
function almacenSoloLectura() {
  const bueno = crearAlmacen();
  globalThis.__CARBOLITAS_STORE__ = {
    ...bueno,
    async escribir(clave, valor) {
      if (clave.startsWith('intentos/')) throw new Error('cuota llena');
      return bueno.escribir(clave, valor);
    },
    async listar(p) {
      return bueno.listar(p);
    }
  };
  return bueno;
}

test('si no se puede contar el intento, no se prueba la contraseña', async () => {
  /* El freno sólo sirve si un fallo lo cierra. Si el contador callara el
     error, quedaría un endpoint sin límite para adivinar la clave. */
  almacenSoloLectura();
  const fns = await funciones();
  const res = await fns.entrar(post('https://x/api/entrar', { password: 'Alitas-al-carbon-2026' }));
  assert.equal(res.status, 503);
  assert.doesNotMatch((await res.json()).error, /incorrecta/);
});

test('sin cabecera de origen en una plataforma real no se atiende el login', async () => {
  montarAlmacen();
  process.env.VERCEL = '1';
  try {
    const fns = await funciones();
    const res = await fns.entrar(post('https://x/api/entrar', { password: 'nope' }));
    assert.equal(res.status, 503);
  } finally {
...
test('no se puede elegir el cubo del contador con una cabecera', async () => {
  montarAlmacen();
  process.env.VERCEL = '1';
  try {
    const fns = await funciones();
    ...
    for (let i = 0; i < 12; i += 1) {
      /* La cabecera que el cliente puede escribir se ignora; la que pone la
         plataforma es la que cuenta. Si se respetara la del cliente, bastaría
         con rotarla para probar contraseñas sin límite. */
      const req = new Request('https://x/api/entrar', {
        method: 'POST',
        headers: {
          'content-type': 'application/json',
          'x-nf-client-connection-ip': `10.0.0.${i}`,
          'x-forwarded-for': `10.0.0.${i}`,
          'x-vercel-forwarded-for': '203.0.113.7'
        },
        body: JSON.stringify({ password: 'nope' })
      });
      ultimo = await fns.entrar(req);
    }
    assert.equal(ultimo.status, 429);
  } finally {
    delete process.env.VERCEL;
...
test('si no se puede leer la revocación, el pase no vale', async () => {
  const bueno = montarAlmacen();
  const fns = await funciones();
  const token = await entrarOk(fns);
  ...
  globalThis.__CARBOLITAS_STORE__ = {
    ...
    async leerTexto(clave) {
      if (clave === 'acceso/revocados-desde') throw new Error('sin conexión');
      return bueno.leerTexto(clave);
    }
  };
  const res = await fns.publicar(post('https://x/api/publicar', { doc: CARTA }, token));
  assert.equal(res.status, 401);
...
test('un ADMIN_SECRET corto se ignora en lugar de debilitar la firma', async () => {
  montarAlmacen();
  process.env.ADMIN_SECRET = 'corto';
  try {
    const fns = await funciones();
    const token = await entrarOk(fns);
    const { createHmac } = await import('node:crypto');
    const [payload, firma] = token.split('.');
    const intento = createHmac('sha256', 'env::corto')
      .update(payload)
      .digest('base64')
      .replace(/\+/g, '-')
      .replace(/\//g, '_')
...
test('las marcas de intentos no se acumulan sin límite', async () => {
  const almacen = montarAlmacen();
  const fns = await funciones();
  for (let i = 0; i < 60; i += 1) {
    await fns.entrar(post('https://x/api/entrar', { password: 'nope' }));
  }
  const marcas = await almacen.listar('intentos/');
  assert.ok(marcas.length <= 40, `quedaron ${marcas.length} marcas`);
});

test('entrar con un pase válido y sin acción no revoca nada por accidente', async () => {
  montarAlmacen();
  const fns = await funciones();
  const token = await entrarOk(fns);
  const res = await fns.entrar(post('https://x/api/entrar', { password: 'x' }, token));
  assert.equal(res.status, 400);
  /* El pase sigue sirviendo: no se ha revocado nada. */
  const sigue = await fns.publicar(post('https://x/api/publicar', { doc: CARTA }, token));
