...
globalThis.__CARBOLITAS_BLOBS__ = () => ({
  async getWithMetadata(key, opts = {}) {
    ...
    return {
      data: opts.type === 'json' ? JSON.parse(v.toString('utf8')) : v.toString('utf8'),
      etag: ETIQUETAS.get(key)
    };
  },
  async get(key, opts = {}) {
    if (!BLOBS.has(key)) return null;
  ...
  async set(key, value) {
    BLOBS.set(key, Buffer.isBuffer(value) ? value : Buffer.from(value));
    ETIQUETAS.set(key, String(++selloQA));
  },
  async setJSON(key, value, opts = {}) {
    if (opts.onlyIfMatch !== undefined && ETIQUETAS.get(key) !== opts.onlyIfMatch) {
      throw new Error('conflicto');
    }
    if (opts.onlyIfNew && BLOBS.has(key)) throw new Error('ya existe');
    BLOBS.set(key, Buffer.from(JSON.stringify(value)));
    ETIQUETAS.set(key, String(++selloQA));
  },
  async delete(key) {
    BLOBS.delete(key);
    ETIQUETAS.delete(key);
  }
});

const FUNCIONES = {
  '/carta.json': (await import('../netlify/functions/carta.mjs')).default,
  '/api/entrar': (await import('../netlify/functions/entrar.mjs')).default,
  '/api/publicar': (await import('../netlify/functions/publicar.mjs')).default,
  '/api/foto': (await import('../netlify/functions/foto.mjs')).default
};
