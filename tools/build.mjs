 ...
 *    build/artifact.html fragmento para publicar como Artifact (no se despliega)
 */
...
/* Fuera de dist/: es un fragmento sin <head> propio y, por tanto, sin la
   política de seguridad que lleva el documento completo. Dentro de la carpeta
   publicada quedaría servido tal cual, siendo la misma página que la carta
   pero sin esa defensa. */
mkdirSync(join(ROOT, 'build'), { recursive: true });
writeFileSync(join(ROOT, 'build/artifact.html'), artifact);
rmSync(join(ROOT, 'dist/artifact.html'), { force: true });
...
/* La carta vigente como archivo: es lo que lee el cliente cuando el sitio se
   ...
   En Netlify la función gana a este archivo con una reescritura forzada. En
   Vercel no: allí el sistema de archivos se consulta ANTES que las
   reescrituras, así que si el archivo existiera, la función nunca respondería
   y publicar desde el panel no cambiaría nada. Por eso el build para Vercel
   no lo escribe. */
const PARA_VERCEL = process.env.CARBOLITAS_TARGET === 'vercel';
if (PARA_VERCEL) rmSync(join(ROOT, 'dist/carta.json'), { force: true });
else writeFileSync(join(ROOT, 'dist/carta.json'), JSON.stringify(BASE_DOC, null, 1));
...
writeFileSync(
  join(ROOT, 'server/carta-base.mjs'),
...
console.log(`build ok
  ...
  build/artifact.html ${kb('build/artifact.html')} KB
