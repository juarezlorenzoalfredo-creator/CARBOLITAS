...
import { guardarFoto, leerFoto } from './store.mjs';
...
export default async (req) => {
  ...
  if (req.method === 'GET') {
    /* Netlify entrega la ruta completa; Vercel reescribe /api/foto/x.webp a
       /api/foto?name=x.webp, porque una reescritura no siempre conserva el
       segmento. Se admiten las dos formas. */
    const nombre = url.searchParams.get('name') || url.pathname.split('/').pop() || '';
