...
export function estadoPassword() {
  ...
  const motivo =
    `El panel no está disponible. Si administras el sitio, revisa en el panel de tu hosting (variables de entorno) que ADMIN_PASSWORD exista y tenga al menos ${MIN_PASSWORD} caracteres.`;
...
export async function tokenValido(token) {
  ...
  /* null = no se pudo leer. No se puede saber si este pase fue revocado, así
     que no se acepta: es preferible pedir la contraseña otra vez a seguir
     aceptando un pase que la dueña creía apagado. */
  if (corteRevocacion === null) return false;
